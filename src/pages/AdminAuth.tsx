...
const Auth = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (signUpStep === 'email') {
      ...
      const { error, data } = await sendAdminSignUpOtp(email.trim(), name.trim());
    ...
    if (signUpStep === 'otp') {
      ...
      const { error } = await verifyAdminSignUpOtp(email.trim(), otp.trim());
    ...
    const { error: profileError } = await completeAdminSignUp(email.trim(), name, password);
